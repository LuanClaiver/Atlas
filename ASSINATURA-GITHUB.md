...
Este procedimento é realizado uma única vez. A chave criada será usada em todos os APKs e AABs futuros do Atlas.

## 1. Criar a senha temporária

No repositório do Atlas, abra **Settings → Secrets and variables → Actions → New repository secret**.
...
2. Na seção **Artifacts**, baixe **Atlas-Assinatura-Permanente**.
3. Extraia o ZIP.
4. Guarde `atlas-upload.jks` em pelo menos dois locais seguros.
...
Abra **Actions → Build Android APK → Run workflow**. O download será disponibilizado como `Atlas-APK-v1.2.6`.
